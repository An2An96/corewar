...
void	ft_live(t_lex_list *args, unsigned int *bytes, t_list_label **labels, char **program)
{
	...
	start = *bytes;
	if (args->lexem->type == DIR && (args->next == NULL || args->next->lexem->type == COMMENT))
	{
		ft_write_program(program, bytes, 1, 0x01);
		ft_write_program(program, bytes, 4, ft_atoi(args->lexem->value));
	}
	else if (args->lexem->type == DIR_LABEL && (args->next == NULL || args->next->lexem->type == COMMENT))
	{
		ft_write_program(program, bytes, 1, 0x01);
...
{
	if (args->lexem->type == REG && (args->next == NULL || args->next->lexem->type == COMMENT))
	{
