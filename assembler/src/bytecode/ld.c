...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/27 16:26:57 by rtroll            #+#    #+#             */
...
static void	ft_write_size(char **program, unsigned int *bytes, unsigned int size, t_lex_list *args)
{
	...
	ft_write_program(program, bytes, 1, ft_atoi(args->next->next->lexem->value));
}

static void	ft_write_direct_label(char **program, unsigned int *bytes, t_list_label **labels, t_lex_list *args)
{
	int start;
	...
	ft_write_program(program, bytes, 1, 0x02);
	ft_write_program(program, bytes, 1, 0x00 | 0b10010000);
	ft_add_to_substitude(labels, 4, *bytes, args->lexem->value)->indexs_to_substitude->start = start;
	ft_write_program(program, bytes, 4, 0xcccccccc);
	ft_write_program(program, bytes, 1, ft_atoi(args->next->next->lexem->value));
}

static void	ft_write_indirect_label(char **program, unsigned int *bytes, t_list_label **labels, t_lex_list *args)
{
	int start;
	...
	ft_write_program(program, bytes, 1, 0x02);
	ft_write_program(program, bytes, 1, 0x00 | 0b11010000);
	ft_add_to_substitude(labels, 2, *bytes, args->lexem->value)->indexs_to_substitude->start = start;
